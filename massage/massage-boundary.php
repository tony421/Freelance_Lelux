		...
		if (!empty($_POST['mode']))
		{
			...
			try {
				...
				if ($mode == 'GET_CONFIG') {
					...
					$result = $massageFunction->voidRecord($recordInfo);
				}
				else if ($mode == 'ADD_RECORD_QUEUE') {
					$recordInfo = $_POST['data'];
					Utilities::logInfo('Massage-Boundary | data[recordInfo]: '.var_export($recordInfo, true));
						
					$result = $massageFunction->addRecordByQueueing($recordInfo);
				}
				else if ($mode == 'ADD_RECORD_BOOKING') {
					$recordInfo = $_POST['data'];
					Utilities::logInfo('Massage-Boundary | data[recordInfo]: '.var_export($recordInfo, true));
						
					$result = $massageFunction->addRecordByBooking($recordInfo);
				}
				else {
